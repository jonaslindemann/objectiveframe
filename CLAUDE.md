...
**Self-weight takes two paths, and a bar needs both.** `addSelfWeightToEq()` puts all three local components of gravity into `Eq`, which is right for a beam because `beam3e` is given all three. A bar has three translation DOFs per node and no transverse distributed-load term, so assembly hands `bar3e` only `Eq`'s axial column — gravity projected onto the bar axis. That projection is a real axial distributed load but it is *not* the bar's weight: alone it applies `n_y²` times too little vertical load (a horizontal bar weighs nothing) and a spurious horizontal `w·n_y·n_x` per unit length. `addBarSelfWeightToLoadVector()` therefore lumps the remainder — gravity minus its axial projection, which is transverse to the bar — onto the two end nodes, half each, in global coordinates. The two parts sum to exactly `(0, -w·L, 0)`. Do not "simplify" this by dropping either half: on a mesh whose diagonals all lean one way (what TetGen's Delaunay gives on a regular grid, the diagonal of a cospherical point set being a tie broken by index order) the spurious horizontal forces add instead of cancelling and the structure visibly leans.

**`recompute()` starts from `m_fElement`, not from zero.** Feedback mode reuses the stiffness matrix `execute()` built and rebuilds only the load vector each frame, so it skips the assembly pass — and with it every equivalent nodal load that pass produced from distributed beam loads and self-weight. `execute()` therefore snapshots the load vector into `m_fElement` at the point where everything in it came from the elements and nothing yet from node loads, and `recompute()` starts from that copy before adding node loads and the feedback force. Move that snapshot and self-weight silently stops acting while a feedback force is dragged (or node loads get counted twice). The invariant to test against: with no feedback force set, `recompute()` must reproduce `execute()` exactly — `objsolver <model> --update-cycle` prints both for comparison.
...
**Interface profiles** — `ofui::UiProfile` (`include/ofui/ui_profile.h`) is a singleton holding which `UiFeature`s are enabled; `UiMode` (SimpleBar/SimpleBeam/Advanced) is only a name for a preset applied in `applyPreset()`. Nothing downstream asks which mode is in force, it asks `profile->has(feature)` — or, for the one thing that is a default rather than a gate, `profile->defaultElementType()` — so another profile is a row in `applyPreset()` rather than an edit at every call site. The two simple presets share the same reduced interface and differ only in what an element is: SimpleBar leaves only the create-bar tool and hides every sectional result beyond normal force, SimpleBeam leaves only the create-beam tool and keeps them all. Toolbar buttons carry their feature as a constructor argument and `ToolbarWindow::doDraw()` skips the ones switched off (collapsing orphaned spacers, while keeping ImGui ids tied to the button index). `FemViewWindow::applyUiMode()` reconciles the rest — hiding panels behind a disabled feature and resetting state the user can no longer reach — and is gated on `UiProfile::revision()` so it is cheap to call every frame. The mode is persisted as the `ui_mode` config value, defaults to SimpleBar with nothing stored, and can be overridden for one run with `--ui-mode=<name>`; `modeFromName()` still reads the pre-split `"simple"` as SimpleBeam so an older stored setting keeps its meaning.
